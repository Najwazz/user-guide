			...
			if (!data.match(/<iframe|<video/gi))
				{
					...
					var youtubeRegexp = /https?:\/\/(?:[0-9A-Z-]+\.)?(?:youtu\.be\/|youtube\.com\S*[^\w\-\s])([\w\-]{11})(?=[^\w\-]|$)(?![?=&+%\w.-]*(?:['"][^<>]*>|<\/a>))[?=&+%\w.-]*/ig;
					var vimeoRegexp = /https?:\/\/(www\.)?vimeo.com\/(\d+)($|\/)/;
					if (data.match(youtubeRegexp))
					{	
						match = youtubeRegexp.exec(data);
						data =  iframeStart + '//www.youtube.com/embed/' + match[1] +iframeEnd;
					}
					else if (data.match(vimeoRegexp))
					{
						match = vimeoRegexp.exec(data);
						data =  iframeStart + '//player.vimeo.com/video/' + match[1] +iframeEnd;
					}
				}
